...
\newcommand{\tree}[2]{\TT_{*, #1}^{#2}[\XX]}
\newcommand{\measure}[2]{\PP_u(\TT_{*, #1}^{#2}[\XX])}
\newcommand{\mSl}{\PP_u(\TT_{*, 2}^{\kappa}[\XX])}
\newcommand{\mEl}{\PP_s(\TT_{*, 1}^{\kappa-1}[\XX]^2)}
\newcommand{\mSs}{\PP_u(\TT_{*, 1}^{\kappa}[\XX])}
\newcommand{\mMs}{\PP_u(\TT_{*, 1}^{\kappa-1}[\XX], \XX)}
\newcommand{\mEs}{\PP_s(\XX^2)}
...
% \begin{proposition}
%     Given symmetric probability measure $\widetilde{\mu}$ on $\mathcal{X}\times\mathcal{X}^\kappa$,
%     \begin{equation*}
%         \pi_{\widetilde{\mu}} = \widetilde{\mu}^{0, 1}
%     \end{equation*}
% \end{proposition}

% \begin{proof}
%     Should be apparent. Not proving because we may not need this result.
% \end{proof}
...
%     For a function $F: \mathcal{X}\times\mathcal{X}^\kappa\mapsto\mathbb{R}$
%     \begin{equation*}
%         \langle F, \widetilde{\xi} \rangle = \int_{\mathcal{X}\times\mathcal{X}^\kappa} F(\widetilde{\tau}) \widetilde{\xi}(d\widetilde{\tau}) = 0
%     \end{equation*}
%     for all signed symmetric measures $\widetilde{\xi}$ on $\mathcal{X}^{\kappa+1}$ such that $\pi_{\widetilde{\xi}} = 0$
%     if and only if
%     \begin{equation*}
%         F(x_0, x_1, \cdots, x_\kappa) = \beta(x_0, x_1, \cdots, x_\kappa) + \frac1\kappa\sum_{v=0}^\kappa\gamma(x_0, x_v)
%     \end{equation*}
%     For some $\gamma: \mathcal{X}^2\mapsto \mathbb{R}$ and $\beta: \mathcal{X}\times\mathcal{X}^\kappa \mapsto \mathbb{R}$ such that for all
%     $\widetilde{\tau}\in\mathcal{X}\times\mathcal{X}^\kappa$, $\sigma\in S_\kappa$,
%     \begin{equation*}
%         \sum_{\sigma\in S_\kappa}\beta(\sigma\widetilde{\tau}) = 0.
%     \end{equation*}
% \end{proposition}
...
%     Consider, using $\widetilde{\xi}(\widetilde{\tau}) = \widetilde{\xi}(\sigma\widetilde{\tau})$
%     \begin{align*}
%         \sum_{\sigma\in S_\kappa} \int_{\mathcal{X}\times\mathcal{X}^\kappa}\beta(\sigma\widetilde{\tau})\widetilde{\xi}(\sigma d\widetilde{\tau})
%         = & \int_{\mathcal{X}\times\mathcal{X}^\kappa}\left[\sum_{\sigma\in S_\kappa} \beta(\sigma\widetilde{\tau})\right]\widetilde{\xi}(\sigma d\widetilde{\tau}) = 0
%         \intertext{On the other hand, taking $\widetilde{\tau}'=\sigma\widetilde{\tau}$}
%         = & \kappa!\int_{\mathcal{X}\times\mathcal{X}^\kappa}\beta(\widetilde{\tau}')\widetilde{\xi}(d\widetilde{\tau}') = \kappa!\langle\beta, \widetilde{\xi}\rangle.
%     \end{align*}
%     Thus, we can deduce that $\langle\beta, \widetilde{\xi}\rangle=0$.

%     For $\gamma$,
%     \begin{align*}
%           & \int_\mathcal{X}\int_{\mathcal{X}^\kappa} \frac1\kappa\sum_{v=1}^\kappa\gamma(x_0, x_v) \widetilde{\xi}(dx_0d\mathbf{x}) \\
%         = & \int_\mathcal{X}\frac1\kappa\sum_{v=1}^\kappa\int_\mathcal{X}\gamma(x_0, x_v)\widetilde{\xi}^{0, v}(dx_0dx_v)
%         \intertext{Note that the $\gamma(x_0, x_v)$ term is not dependent on $v$}
%         = & \int_{\mathcal{X}^2}\gamma(x_0, x_v)\left[\frac1\kappa\sum_{v=1}^\kappa\widetilde{\xi}(dx_0dx_v)\right]                  \\
%         = & \int_{\mathcal{X}^2}\gamma(x_0, x_v)\pi_{\widetilde{\xi}}(dx_0dx_v)= 0
%     \end{align*}

%     For the forward direction, we will show that $\gamma(x, y) = F(x, y, \cdots, y)$, and $\beta(x_0, \mathbf{x})=F(x_0, \mathbf{x}) -
%         \frac1\kappa\sum_{v=1}^\kappa\gamma(x_0, x_v)$. Note that $\beta(x, y, \cdots, y) = 0$.
%     We prove $\sum_{\sigma}\beta(x_0, \sigma\mathbf{x}) = 0$ by contradiction.

%     First, consider, for all $E\subseteq \mathcal{X}\times\mathcal{X}^\kappa$ measurable,
%     \begin{equation*}
%         \widetilde{\xi}_{(x_0, \mathbf{x})}(E) := \mathbf{1}_{\{(x_0, \mathbf{x})\in E\}} - \frac1\kappa\sum_{v=1}^\kappa\mathbf{1}_{\{(x_0, (x_v, \cdots, x_v))\in E\}}
%     \end{equation*}
%     and
%     \begin{equation*}
%         \widetilde\xi(E) := \sum_{\sigma\in S_\kappa}\widetilde{\xi}_{\sigma\widetilde{\tau}}(E).
%     \end{equation*}
%     Notice that we can simplify $\eta$:
%     \begin{align*}
%         \eta(x_0', \mathbf{x}') & = \sum_{\sigma\in S_\kappa}\mathbf{1}_{\{(x_0, \sigma\mathbf{x})\in E\}}
%         - \kappa!\frac1\kappa\sum_{v=1}^\kappa\mathbf{1}_{\{(x_0, \sigma(x_v, \cdots, x_v))\in E\}}
%         \intertext{Take $v' = \sigma^{-1}(v)$, }
%                                 & = \sum_{\sigma\in S_\kappa}\mathbf{1}_{\{(x_0, \sigma\mathbf{x})\in E\}}
%         - \kappa!\frac1\kappa\sum_{v'=1}^\kappa \mathbf{1}_{\{(x_0, (x_{v'}, \cdots, x_{v'}))\in E\}}      \\
%                                 & = \sum_{\sigma\in S_\kappa}\mathbf{1}_{\{(x_0, \sigma\mathbf{x})\in E\}}
%         - \kappa!\frac1\kappa\sum_{v=1}^\kappa\mathbf{1}_{\{(x_0, (x_v, \cdots, x_v))\in E\}}.
%     \end{align*}

%     We first check that $\eta$ is symmetric: for some $\sigma'\in S_\kappa$, $E\subseteq\mathcal{X}\times\mathcal{X}^\kappa$ measurable,
%     \begin{align*}
%         \widetilde{\xi}(\sigma' E) & = \sum_{\sigma\in S_\kappa}\mathbf{1}_{\{(x_0, \sigma\mathbf{x})\in \sigma'E\}}
%         - \kappa!\frac1\kappa\sum_{v=1}^\kappa\mathbf{1}_{\{(x_0, (x_v, \cdots, x_v))\in \sigma'E\}}
%         \intertext{Subsititute $\sigma'' = \sigma'^{-1} \circ \sigma$, noting that $\widetilde{\tau}\in \sigma'E$ if and only if
%         $\sigma'^{-1}\widetilde{\tau}\in E$,}
%                                    & = \sum_{\sigma''\in S_\kappa}\mathbf{1}_{\{(x_0, \sigma''\mathbf{x})\in E\}}
%         - \kappa!\frac1\kappa\sum_{v=1}^\kappa\mathbf{1}_{\{(x_0, \sigma'^{-1}(x_v, \cdots, x_v))\in E\}}            \\
%                                    & = \sum_{\sigma\in S_\kappa}\mathbf{1}_{\{(x_0, \sigma\mathbf{x})\in E\}}
%         - \kappa!\frac1\kappa\sum_{v=1}^\kappa\mathbf{1}_{\{(x_0, (x_v, \cdots, x_v))\in E\}}  = \widetilde{\xi}(E)
%     \end{align*}
%     On the other hand, to prove that $\pi_{\widetilde{\xi}}=0$, fix $E\subseteq \mathcal{X}^2$ measurable
%     \begin{align*}
%         \pi_{\widetilde{\xi}_{(x_0, \mathbf{x})}}(E) = \frac1\kappa\sum_{v=1}^\kappa
%         \widetilde{\xi}^{0, v}_{(x_0, \mathbf{x})}(E) & = \frac1\kappa\sum_{v=1}^\kappa \left[\mathbf{1}_{\{(x_0, x_v)\in E\}} - \frac1\kappa\sum_{u=1}^\kappa\mathbf{1}_{\{(x_0, x_u)\in E\}}\right] \\
%                                                       & = \frac1\kappa\sum_{v=1}^\kappa \mathbf{1}_{\{(x_0, x_v)\in E\}} - \frac1\kappa\sum_{u=1}^\kappa\mathbf{1}_{\{(x_0, x_u)\in E\}} = 0
%     \end{align*}
%     Hence, $\pi_{\widetilde{\xi}}(E) = \sum_{\sigma\in S_\kappa}\pi_{\widetilde{\xi}_{\sigma\widetilde\tau}}(E) = 0$.

%     Assume that for some $\widetilde\tau = (x_0, \mathbf{x})\in\mathcal{X}\times\mathcal{X}^\kappa$, $\sum_{\sigma}\beta(\sigma\widetilde{\tau}) \neq 0$, we
%     will show that $\langle\beta, \widetilde{\xi}\rangle\neq 0$. Then,
%     \begin{align*}
%           \int_{\mathcal{X}^{\kappa+1}} \beta(x_0, \mathbf{x})\widetilde{\xi}(dx_0'd\mathbf{x}')                                     
%         = \sum_{\sigma\in S_\kappa}\beta(x_0, \sigma\mathbf{x}) - \kappa!\frac1\kappa\sum_{v=1}^\kappa\beta(x_0, (x_v, \cdots, x_v)) 
%         = \sum_{\sigma\in S_\kappa}\beta(x_0, \sigma\mathbf{x}) \neq 0
%     \end{align*}

%     $\langle\beta, \widetilde{\xi}\rangle \neq 0$, combined with $\langle\frac1\kappa\sum \gamma(\cdot), \widetilde{\xi}\rangle = 0$, by
%     linearity, we can get $\langle F, \eta\rangle\neq 0$, which is a contradiction. This means that $\beta$ has to satisfy $\sum_\sigma
%         \beta(x_0, \sigma(\mathbf{x})) = 0$. This means that $F$ has to come in the form
%     \begin{equation*}
%         F(x_0, x_1, \cdots, x_\kappa) = \beta(x_0, x_1, \cdots, x_\kappa) + \frac1\kappa\sum_{v=0}^\kappa\gamma(x_0, x_v),
%     \end{equation*}
...
\begin{proposition}
    ...
    % \begin{enumerate}
    %     \item For all $\tau\in\tree{2}{\kappa}$
    %           \begin{equation}
    %               \widehat{\rho}_2(\tau) = \rho_1(\tau_1)\prod_{v\in N_o(\tau)}\frac{\pi_{\rho_2}(\tau(v\backslash o)_1, \tau(o\backslash v)_1)}{\pi_{\rho_1}(\tau(v\backslash o)_0, \tau_0)\widehat{P}_{1, \rho_1}(\tau_0, \tau(v\backslash o)_0)(\tau(o\backslash v)_1)},
    %           \end{equation}
    %     \item and
    %           \begin{equation}
    %               \pi_{\widehat{\rho}_2} = \pi_{\rho_2}
    %           \end{equation}
    % \end{enumerate}
\end{proposition}

\begin{proof}
    % The first result can be obtained by
    % \begin{equation*}
    %     \pi_{\rho_2^*}(\tau(v\backslash o)_1, \tau(o\backslash v)_1) = \pi_{\rho_1}(\tau(v\backslash o)_0, \tau_0)
    %     \widehat{P}_{1,\rho_1}(\tau(v\backslash o)_0, \tau_0)(\tau(v\backslash o)_1)
    %     \widehat{P}_{1,\rho_1}(\tau_0, \tau(v\backslash o)_0)(\tau(o\backslash v)_1)
    % \end{equation*}
...
\begin{multline*}
    \AP(\PU) = \sup_{\Sl\in\mSl} \left\{\EE_\Sl\left[\frac12\sum_{v\in\NB{\TB}}\log\psi^1(\TB_o, \TB_v)+\frac12\sum_{v\in\NB{\TB}}\sum_{u\in\NB{\TB(v\BS o)_1}}\log\PU(\tau_o, \tau(u\BS v)_0)\right] \right.\\
...
\begin{lemma}\label{Prp.NNNI-EV}
    \begin{enumerate}
        \item For $\Sl: \tree{2}{\kappa}\mapsto\RR$ and $f: \tree{1}{\kappa}\times \XX \mapsto\RR$, let
              \begin{align*}
                  \Ss(\tau) = \sum_{\tau'\in\tree{2}{\kappa}}\Sl(\tau')\one{\tau'_1=\tau},
              \end{align*}
              we have
              \begin{equation}\label{Eq.NNNI-EV-SL-SS}
                  \sum_{\tau\in\tree{2}{\kappa}} \Sl(\tau)\sum_{v\in\NB{\tau}}f(\tau(v\BS o)_1, \tau_o) = \sum_{\tau\in\tree{1}{\kappa}} \Sl(\tau) \sum_{v\in\NB{\tau}} f(\tau(o\BS v)_1, \tau_v)
              \end{equation}
        \item For $\Ss: \tree{1}{\kappa}\mapsto\RR$ and $g: \XX^2\mapsto\RR$, let
              \begin{align*}
                  \Es(x, x') = \sum_{\tau'\in\tree{1}{\kappa}}\Ss(\tau')E_1(x, x')(\tau),
              \end{align*}
              we have
              \begin{equation}\label{Eq.NNNI-EV-SS-ES}
                  \sum_{\tau\in\tree{1}{\kappa}} \Ss(\tau)\sum_{v\in\NB{\tau}}g(\tau_v, \tau_o) = \sum_{x, x'\in\XX} \El(x, x') \kappa g(x, x')
              \end{equation}
    \end{enumerate}
\end{lemma}
Apply (\ref{Eq.NNNI-EV-SL-SS}) with $f(\tau, x) = \frac12 \log\psi^1(\tau_o, x) + \frac12 \sum_{u\in\NB{\tau}}\log\psi^2(\tau_u, x)$
\begin{equation}
    \begin{aligned}
          & \EE_\Sl\left[\frac12\sum_{v\in\NB{\TB}}\log\psi^1(\TB_o, \TB_v) + \frac12\sum_{v\in\NB{\TB}}\sum_{u\in\NB{\tau(v\BS o)_1}}\log\psi^2(\TB_o, \TB(u\BS v)_0)\right] \\
        % = & \EE_\Sm\left[\frac12\log\PO(\tau_o, x)+\frac12\sum_{v\in\NB{\tau}}\log\PO(\tau_o, \tau(v\BS o)_0)+\frac\kappa2\sum_{v\in\NB{\tau}}\log\PU(x, \tau(v\BS o)_0)\right]       \\
        = & \EE_\Ss\left[\frac12\sum_{v\in\NB{\TB}}\log\psi^1(\TB_o, \TB_v)+\frac12\sum_{\substack{v, u\in\NB{\TB}                                                            \\v\neq u}}\log\psi^2(\TB_v, \TB_u)\right].
    \end{aligned}
\end{equation}
Consider $\Sl$ and $\Ss$ as two variables with constraint $\Ss(\tau) = \sum_{\tau'}\Sl(\tau')\one{\tau'_1=\tau}$ for all $\tau\in\tree{1}{\kappa}$. We can collect all terms relating to $\Ss$ and optimizer over $\Sl$ separately, giving the optimization problem \footnote{Rereading this... maybe I should
    have explained it a little more?}
\begin{multline*}
    \AP(\PU) = \sup_{\Ss\in\mSs} \left\{\EE_\Ss\left[\frac12\sum_{v\in\NB{\TB}}\log\psi^1(\TB_o, \TB_v)+\sum_{\{v, u\}\in\NB{\TB}}\log\psi^2(\TB_v, \TB_u)\right] \right.\\
    \left.- \left[H(\Ss\|\SUs) - \frac{\kappa}{2} H(\Es\|\EUs)\right] - \inf_{\substack{\Sl\in\mSl\\ \Ss(\tau) = \sum_{\tau'}\Sl(\tau')\one{\tau'_1=\tau}}}\left[H(\Sl\|\Slo) - \frac\kappa2 H(\El\|\Elo)\right]\right\}
\end{multline*}
...
\begin{lemma}\label{Prp.NNNI-NU-RHO}
    ...
    % \item Relation to $\El$:
    %       \begin{equation}
    %           \Sm(\tau, x) = \sum_{\tau'\in\tree{1}{\kappa-1}} \El(\tau, \tau')\one{\tau'_o=x} = \Es(\tau_o, x)\UE(\tau_o, x)(\tau)
    %       \end{equation}
    % \item Relation to $\Es$:
    %       \begin{equation}
    %           \Es(x, x') = \sum_{\tau\in\tree{1}{\kappa-1}} \Sm(\tau, x)\one{\tau_o=x'} = \sum_{\tau,\tau'\in\tree{1}{\kappa-1}} \El(\tau, \tau')\one{\tau_o=x, \tau_o=x'}
    %       \end{equation}
    % \item Relation to $\Ss$:
    %       \begin{equation}
    %         \Ss(\tau) = \sum_{v\in\NB{\tau}}\frac{\Sm(\tau(o\BS v)_1, \tau(v\BS o)_0)}{n_{\tau(v\BS o)_0}(\tau)}
    %       \end{equation}
    ...
    %       \end{equation}
    % \end{enumerate}
    For $\Sl\in\mSl$ and $\Ss\in\mSs$, define $\Sm(\tau, x) = \frac1\kappa \cnt{x}(\tau\oplus x) \Ss(\tau\oplus x)$ and $\El(\tau, \tau) = \EE_\Sl\left[\frac1\kappa E_2(\tau,\tau')(\TB)\right]$.
    \begin{align}
        \Ss(\tau)    & = \sum_{\tau'\in\tree{2}{\kappa}}\Sl(\tau')\one{\tau'_1=\tau}      & \forall \tau\in\tree{1}{\kappa}
        \intertext{if and only if}
        \Sm(\tau, x) & = \sum_{\tau'\in\tree{1}{\kappa-1}}\El(\tau, \tau')\one{\tau'_0=x} & \forall \tau\in\tree{1}{\kappa-1}, x\in\XX.
    \end{align}
...
\begin{multline*}
    \VAL(\Ss) = \EE_\Ss\left[\frac12\sum_{v\in\NB{\TB}}\log\psi^1(\TB_o, \TB_v)+\sum_{\{v, u\}\subseteq\NB{\TB}}\log\psi^2(\TB_v, \TB_u)\right] \\
    - \left[H(\Ss\|\SUs) - \frac{\kappa}{2} H(\Es\|\EUs)\right].
\end{multline*}
For simplicity, we define, for $\tau$ a $\kappa$-star,
\begin{equation}
    \psi(\tau) = \prod_{v\in\NB{\tau}}\left[\psi^1(\tau_o, \tau_v)\right]^{1/2}\prod_{\{v, u\}\subseteq\NB{\tau}}\log\psi^2(\tau_v, \tau_u),
\end{equation}
giving
\begin{equation}
    \VAL(\Ss) = \EE_\Ss\log\psi(\TB) - \left[H(\Ss\|\SUs) - \frac{\kappa}{2} H(\Es\|\EUs)\right].
\end{equation}
And thus
\begin{equation*}
    \AP(\PU) = \sup_{\substack{\Ss\in\mSs,\ \Es\in\mEs\\ \EE_\Ss[E_1(x,x')(\TB)]=\kappa\Es(x, x')}} \VAL(\Ss, \Es)
\end{equation*}
Use Lagrange multiplier to find the stationary points of $\VAL(\Ss, \Es)$ in the domain. We need the following constraints:
\begin{align*}
    \sum_{\tau\in\tree{1}{\kappa}}\Ss(\tau)          & = 1                                             \\
    \Ss(\tau)                                        & \geq 0       & \forall \tau\in \tree{1}{\kappa} \\
    \EE_\Ss\left[\frac1\kappa E_2(x, x')(\TB)\right] & = \Es(x, x') & \forall x, x'\in\XX              \\
    \Es(x, x')                                       & = \Es(x', x) & \forall x, x'\in\XX              \\
    \sum_{x,x'\in\XX}\Es(x, x')                      & = 1
\end{align*}
Note that this guarantees that $\Es$ is a symmetric probability measure on $\XX^2$. It is non-negative since $E_2$ is non-negative.
Additionally, note that the first constraint is redundant:
\begin{lemma}\label{Prp.NNNI-SS-NORM}
    For $\Ss$ a function on $\kappa$-regular stars, let $\Es(x, x')=\sum_\tau \Ss(\tau) E_1(x, x')(\tau)$, have
    \begin{equation*}
        \sum_{\tau\in\tree{1}{\kappa}}\Ss(\tau) = \sum_{x, x'\in\XX} \Es(x, x')
    \end{equation*}
\end{lemma}
\begin{proof}
    Apply (\ref{Eq.NNNI-EV-SS-ES}) with $g(x, x') = 1/\kappa$
    \begin{equation*}
        \sum_{\tau\in\tree{1}{\kappa}}\Ss(\tau) = \sum_{\tau\in\tree{1}{\kappa}}\Ss(\tau) \sum_{v\in\NB{\tau}} \frac1\kappa  = \sum_{x, x'\in\XX}\Es(x, x')\ \kappa\ \frac1\kappa = \sum_{x, x'\in\XX}\Es(x, x')
    \end{equation*}
\end{proof}

Solving the Lagrange multiplier problem allows us to find the ``cavity functional'', an important functional that underlies all stationary points of $\VAL$. It is a functional on probability measures on $\XX^2$.
\begin{equation}
    \begin{aligned}
        \BP: \PP(\XX^2)                                         & \mapsto \PP(\XX^2)                                                                                \\
        \CF(x, x')                                              & \mapsto \frac{1}{z_\CF}\sum_{\tau\in\tree{1}{\kappa-1}} \one{\tau_o=x}{\kappa-1\choose\CNT(\tau)}
        \psi(\tau\oplus x')\prod_{v\in\NB{\tau}} \CF(\tau_v, x) & \forall x, x'\in\XX^2
    \end{aligned}
\end{equation}
where $z_\CF$ normalizes the distribution:
\begin{equation}
    z_\CF = \sum_{x, x'\in\XX}\sum_{\tau\in\tree{1}{\kappa-1}} \one{\tau_o=x}{\kappa-1\choose\CNT(\tau)}
    \psi(\tau\oplus x')\prod_{v\in\NB{\tau}} \CF(\tau_v, x).
\end{equation}
...
\begin{proposition}\label{Prp.NNNI-CF}
    The annealed pressure of next-nearest-neighbor interation model with positive specification $\PU = (\psi^1, \psi^2)$,
    \begin{equation}
        \AP(\PU) = \sup_{\substack{h^*\in\PP(\XX^2)\\ \CFo = \BP\CFo}} \left(\log z_\Ss - \frac\kappa2 \log z_\Es\right),
    \end{equation}
    where
    \begin{align}
        z_\Ss = & \sum_{\tau\in\tree{1}{\kappa}}{\kappa\choose\CNT(\tau)}\psi(\tau)\prod_{v\in\NB{\tau}}h(\tau_v, \tau_o) \\
        z_\Es=  & \sum_{x, x'\in\XX}\CF(x, x')\CF(x', x).
    \end{align}
\end{proposition}
Note that $z_\Ss$ and $z_\Es$ are the normalizing constants for the stationary points of $\VAL(\Ss, \Es)$,
\begin{align}
    \Ss(\tau)  & = \frac{1}{z_\Ss}{\kappa\choose\CNT(\tau)}\psi(\tau)\prod_{v\in\NB{\tau}}h(\tau_v, \tau_o) \\
    \Es(x, x') & = \frac{1}{z_\Es}\CF(x, x')\CF(x', x),
\end{align}
...
\begin{proof}[Proof of (\ref{Prp.NNNI-EV})]
    ...
    %     = & \EE_\Sl\left[\frac12\sum_{v\in\NB{\tau}}\log\PO(\tau_o, \tau(v\BS o)_0)\right]
    % \end{align*}
    To prove (\ref{Eq.NNNI-EV-SS-ES}), the unimodularity of $\Sl$ is required, so we add an intermediate step involving $\El$.
    \begin{align*}
        \EE_\Sl\left[\frac12\sum_{v\in\NB{\tau}}\sum_{u\in\NB{\tau(v\BS o)_1}}\log\PU(\tau_o, \tau(u\BS v)_0)\right]
    ...
    \begin{align*}
        ...
        = & \sum_{\TB}\Sl(\TB)\sum_{\tau,\tau'\in\tree{1}{\kappa-1}}\frac1\kappa E_2(\tau,\tau')(\TB)\left[\frac\kappa2\sum_{v\in\NB{\tau}}\log\PU(\tau'_o, \tau(v\BS o)_0)\right]                                          \\
        = & \sum_{\TB}\Sl(\TB)\sum_{\tau,\tau'\in\tree{1}{\kappa-1}} E_2(\tau,\tau')(\TB)\left[\frac12\sum_{v\in\NB{\tau}}\log\PU(\tau'_o, \tau(v\BS o)_0)\right]                                                           \\
        = & \sum_{\TB}\Sl(\TB)\sum_{v\in\NB{\TB}}\sum_{\tau,\tau'\in\tree{1}{\kappa-1}} \one{\TB(v\BS o)_1=\tau, \TB(o\BS v)_1=\tau'}\left[\frac12\sum_{v\in\NB{\tau}}\log\PU(\tau'_o, \tau(v\BS o)_0)\right]               \\
        = & \sum_{\TB}\Sl(\TB)\sum_{v\in\NB{\TB}}\sum_{\tau,\tau'\in\tree{1}{\kappa-1}} \one{\TB(v\BS o)_1=\tau, \TB(o\BS v)_1=\tau'}\left[\frac12\sum_{u\in\NB{\TB(v\BS o)_1}}\log\PU(\TB_o, \TB(v\BS o)(u\BS o)_0)\right] \\
        = & \sum_{\TB}\Sl(\TB)\sum_{v\in\NB{\TB}} \left[\frac12\sum_{u\in\NB{\TB(v\BS o)_1}}\log\PU(\TB_o, \TB(v\BS o)(u\BS o)_0)\right]                                                                                    \\
        = & \sum_{\TB}\Sl(\TB) \left[\frac12\sum_{v\in\NB{\TB}}\sum_{u\in\NB{\TB(v\BS o)_1}}\log\PU(\TB_o, \TB(u\BS v)_0)\right]                                                                                            \\
        = & \EE_\Sl\left[\frac12\sum_{v\in\NB{\tau}}\sum_{u\in\NB{\tau(v\BS o)_1}}\log\PU(\tau_o, \tau(u\BS v)_0)\right].
    \end{align*}
    On the other hand,
    \allowdisplaybreaks{
        \begin{align*}
              & \EE_\El\left[\frac\kappa2\sum_{v\in\NB{\tau}}\log\PU(\tau'_o, \tau(v\BS o)_0)\right]                                                                                                                                    \\
            = & \sum_{\tau,\tau'\in\tree{1}{\kappa-1}}\El(\tau, \tau')\left[\frac\kappa2\sum_{v\in\NB{\tau}}\log\PU(\tau'_o, \tau(v\BS o)_0)\right]
            \intertext{By the unimodularity of $\Sl$, $\El$ is symmetric, so}
            = & \sum_{\tau,\tau'\in\tree{1}{\kappa-1}}\El(\tau', \tau)\left[\frac\kappa2\sum_{v\in\NB{\tau}}\log\PU(\tau'_o, \tau(v\BS o)_0)\right]                                                                                     \\
...
\begin{proof}[Proof of (\ref{Prp.NNNI-CF})]
    ...
    \begin{align*}
        \EE_\Ss\left[\frac1\kappa E_1(x, x')(\TB)\right] & = \Es(x, x') & \alpha(x, x') \\
        \Es(x, x')                                       & = \Es(x', x) & \beta(x, x')  \\
        \sum_{x,x'\in\XX}\Es(x, x')                      & = 1          & \gamma        \\
        -\Ss(\tau)                                       & \leq 0       & \delta(\tau),
    \end{align*}
    and $\delta(\tau)\geq 0$, $\rho(\tau)\delta(\tau) = 0$ for all $\tau\in\tree{1}{\kappa}$.
    We have
    \begin{multline*}
        \LM(\Ss, \Es, \alpha, \beta, \gamma, \delta) = \EE_\Ss\log\psi(\TB) - \left[H(\Ss\|\SUs) - \frac{\kappa}{2} H(\Es\|\EUs)\right] \\
        + \sum_{x, x'\in\XX}\alpha(x, x')\left\{\EE_\Ss\left[\frac1\kappa E_1(x,x')(\TB)\right] - \Es(x,x')\right\}
        + \sum_{x, x'\in\XX} \beta(x, x')\left[\Es(x, x')-\Es(x',x)\right] \\
        + \gamma\left[\sum_{x, x'\in\XX} \Es(x, x') - 1\right] - \sum_{\tau\in\tree{1}{\kappa}}\delta(\tau)\Ss(\tau).
    \end{multline*}
    Taking derivative with respect to $\Ss$ and $\Es$. Obtain, for all $\tau\in\tree{1}{\kappa}$
    \begin{equation}\label{Eq.NNNI-LM-SS}
        \frac{\partial \LM(\Ss, \Es, \alpha, \beta, \gamma)}{\partial \Ss(\tau)} = \log\psi(\tau)
        - \left[\log \frac{\Ss(\tau)}{\SUs(\tau)} - 1\right]
        + \sum_{x, x'\in\XX}\alpha(x, x')\frac1\kappa E_1(x, x')(\tau) - \delta(\tau) = 0,
    \end{equation}
    and for all $x, x'\in\XX$
    ...
    \begin{align*}
        \sum_{x, x'\in\XX}\alpha(x, x')\frac1\kappa E_1(x, x')(\tau) & = \sum_{x, x'\in\XX}\alpha(x, x')\frac1\kappa \sum_{v\in\NB{\tau}} \one{\tau_v=x, \tau_o=x'}          \\
                                                                     & = \sum_{v\in\NB{\tau}} \sum_{x, x'\in\XX}\alpha(x, x')\frac1\kappa \one{\tau_v=x, \tau_o=x'}          \\
                                                                     & = \sum_{v\in\NB{\tau}} \sum_{x, x'\in\XX}\alpha(\tau_v, \tau_o)\frac1\kappa \one{\tau_v=x, \tau_o=x'} \\
                                                                     & = \sum_{v\in\NB{\tau}} \frac1\kappa\alpha(\tau_v, \tau_o),                                            \\
    \end{align*}
    so
    \begin{align*}
        \log \frac{\Ss(\tau)}{\SUs(\tau)} - 1 & = \log\psi(\tau) + \sum_{v\in\NB{\tau}} \frac1\kappa\alpha(\tau_v, \tau_o) -\delta(\tau)                                \\
        \log \frac{\Ss(\tau)}{\SUs(\tau)}     & = \log\psi(\tau) + \sum_{v\in\NB{\tau}}\left[\frac1\kappa\alpha(\tau_v, \tau_o) + \frac1\kappa\right] - \delta(\tau)    \\
        \frac{\Ss(\tau)}{\SUs(\tau)}          & = \psi(\tau)\prod_{v\in\NB{\tau}}\exp\left[ \frac1\kappa\alpha(\tau_v, \tau_o) + \frac1\kappa\right]e^{ - \delta(\tau)}
    \end{align*}
    Define the probability measure $\CF$ on $\XX^2$ with the following pmf:
    \begin{equation}\label{Eq.NNNI-CF-ALPHA}
        \CF(x, x') = \frac{1}{\overline{z_\CF}}\exp\left[\frac1\kappa\alpha(x, x') + \frac1\kappa\right]
    \end{equation}
    so
    \begin{align*}
        \Ss(\tau) & = \SUs(\tau)\overline{z_\CF}^\kappa\psi(\tau)e^{-\delta(\tau)}\prod_{v\in\NB{\tau}}\CF(\tau_v, \tau_o)
        \intertext{Notice that for all $\tau$, the $\psi(\tau)>0$, and the exponential is also strictly positive, meaning that $\Ss(\tau)>0$, and thus $\delta(\tau)=0$, so}
        \Ss(\tau) & = \SUs(\tau)\overline{z_\CF}^\kappa\psi(\tau)\prod_{v\in\NB{\tau}}\CF(\tau_v, \tau_o)
        \intertext{Notice that since $\SUs(\tau) = {\kappa\choose\CNT(\tau)}/|\XX|^{\kappa+1}$\footnote{Probably should have proven this ahead of time}}
        \Ss(\tau) & = \frac{\overline{z_\CF}^\kappa}{|\XX|^{\kappa+1}}{\kappa\choose\CNT(\tau)}\psi(\tau)\prod_{v\in\NB{\tau}}\CF(\tau_v, \tau_o).
    \end{align*}
    And
    \begin{equation}\label{Eq.NNNI-SS}
        \Ss(\tau) = \frac{1}{z_\Ss}{\kappa\choose\CNT(\tau)}\psi(\tau)\prod_{v\in\NB{\tau}}\CF(\tau_v, \tau_o),
    \end{equation}
    with
    \begin{equation}\label{Eq.NNNI-ZSS-ZCF}
        z_\Ss=\frac{|\XX|^{\kappa+1}}{\overline{z_\CF}^\kappa}.
    \end{equation}
    ...
    \begin{align*}
        ...
        \intertext{By $\EUs(x, x')=1/|\XX|^2$,}
        \Es(x, x')                                                    & = \frac1{|\XX|^2}\exp\left[\frac1\kappa\alpha(x, x') + \frac1\kappa\alpha(x', x) - \frac{2\gamma}{\kappa} + 1\right]                                                                        \\
        \intertext{Similar to $\Ss$, we rewrite $\alpha$ in terms of $\CF$}
                                                                      & = \frac1{|\XX|^2}\exp\left[\left(\frac1\kappa\alpha(x, x') + \frac1\kappa\right) + \left(\frac1\kappa\alpha(x', x) + \frac1\kappa\right) - \frac{2\gamma}{\kappa} + 1 - \frac2\kappa\right] \\
        \intertext{By definition of $\CF$ in (\ref{Eq.NNNI-CF-ALPHA}),}
                                                                      & = \frac{1}{|\XX|^2}\overline{z_\CF}\CF(x, x') \overline{z_\CF}\CF(x', x)\exp\left[ - \frac{2\gamma}{\kappa} + 1 - \frac2\kappa\right]                                                       \\
                                                                      & = \frac{\overline{z_\CF}^2}{|\XX|^2}\exp\left[ - \frac{2\gamma}{\kappa} + 1 - \frac2\kappa\right]\CF(x, x')\CF(x', x).
    \end{align*}
    Combine the constants by defining $z_\Es = |\XX|^2/\overline{z_\CF}^2\exp\left[1-\frac{2}{\kappa}(\gamma+1)\right]$\footnote{Probably should have proven this ahead of time}
    \begin{equation}
        \Es(x, x') = \frac1{z_\Es}\CF(x, x')\CF(x', x),
    \end{equation}
    ...
    \begin{equation}\label{Eq.NNNI-ZES}
        z_\Es = \sum_{x, x'\in\XX}\CF(x, x')\CF(x', x).
    \end{equation}
    For the remaining constraint,
    \allowdisplaybreaks{
        \begin{align*}
              & \EE_\Ss\left[\frac1\kappa E_1(x, x')(\TB)\right]                                                                                                                                                                                                           \\
            = & \sum_{\tau\in\tree{1}{\kappa}}\Ss(\tau)\frac1\kappa E_1(x, x')(\tau)                                                                                                                                                                                       \\
            = & \sum_{\tau\in\tree{1}{\kappa}}\frac1\kappa E_1(x, x')(\tau)\frac{1}{z_\Ss}{\kappa\choose\CNT(\tau)}\psi(\tau)\prod_{v\in\NB{\tau}}\CF(\tau_v, \tau_o)                                                                                                      \\
            = & \frac{1}{z_\Ss}\sum_{\tau\in\tree{1}{\kappa}}\frac1\kappa \sum_{w\in\NB{\tau}}\one{\tau_w=x, \tau_o=x'}{\kappa\choose\CNT(\tau)}\psi(\tau)\prod_{v\in\NB{\tau}}\CF(\tau_v, \tau_o)
            \intertext{Introduce $\tau'=\tau(v\BS o)_1$}
            = & \frac{1}{z_\Ss}\sum_{\tau\in\tree{1}{\kappa}}\frac1\kappa \sum_{w\in\NB{\tau}}\sum_{\tau'\in\tree{1}{\kappa-1}}\one{\tau(w\BS o)_1=\tau'}\one{\tau_w=x, \tau_o=x'}{\kappa\choose\CNT(\tau)}                                                                \\
              & \psi(\tau)\prod_{v\in\NB{\tau}}\CF(\tau_v, \tau_o)
            \intertext{Rewrite everything in terms of $\tau'$}
            = & \frac{1}{z_\Ss}\sum_{\tau\in\tree{1}{\kappa}}\frac1\kappa \sum_{w\in\NB{\tau}}\sum_{\tau'\in\tree{1}{\kappa-1}}\one{\tau=\tau'\oplus x}\one{\tau_w=x, \tau'_o=x'}{\kappa\choose\CNT(\tau'\oplus x)}                                                        \\
              & \psi(\tau'\oplus x)\CF(x, x')\prod_{v\in\NB{\tau'}}\CF(\tau'_v, x')
            \intertext{Collect all terms still written in terms of $\tau$}
            = & \frac{1}{z_\Ss}\sum_{\tau'\in\tree{1}{\kappa-1}}\sum_{\tau\in\tree{1}{\kappa}}\one{\tau=\tau'\oplus x, \tau'_o=x'}\frac1\kappa \sum_{w\in\NB{\tau}}\one{\tau_w=x}\psi(\tau'\oplus x)\CF(x, x')\prod_{v\in\NB{\tau'}}\CF(\tau'_v, x')
            \intertext{Notice that $\sum_{w\in\NB{\tau}}\one{\tau_w=x}=\cnt{x}(\tau)$}
            = & \frac{1}{z_\Ss}\sum_{\tau'\in\tree{1}{\kappa-1}}\sum_{\tau\in\tree{1}{\kappa}}\one{\tau=\tau'\oplus x, \tau'_o=x'}\frac{\cnt{x}(\tau'\oplus x)}\kappa{\kappa\choose\CNT(\tau'\oplus x)}\psi(\tau'\oplus x)\CF(x, x')\prod_{v\in\NB{\tau'}}\CF(\tau'_v, x')
            \intertext{So now we can remove $\tau$}
            = & \frac{1}{z_\Ss}\sum_{\tau'\in\tree{1}{\kappa-1}}\one{\tau'_o=x'}\frac{\cnt{x}(\tau'\oplus x)}\kappa{\kappa\choose\CNT(\tau'\oplus x)}\psi(\tau'\oplus x)\CF(x, x')\prod_{v\in\NB{\tau'}}\CF(\tau'_v, x')
        \end{align*}}
    Focusing on the multiplicity, we see that\footnote{Probably should have proven this ahead of time}
    \begin{align*}
        {\kappa\choose\CNT(\tau'\oplus x)} & = \frac{\kappa!}{\prod{x'\in\XX}\cnt{x'}(\tau'\oplus x)!}
        \intertext{Notice that in the denominator, only $\cnt{x}(\tau'\oplus x)!$ is one more than the others, otherwise the other terms look exactly like that of $\CNT(\tau')$. Extract the term}
                                           & = \frac{\kappa!}{\prod_{x'\in\XX}\left[\cnt{x'}(\tau') + \one{x'=x}\right]!}         \\
                                           & = \frac{\kappa!}{(\cnt{x}+1)\prod_{\substack{x'\in\XX}}\left[\cnt{x}(\tau')\right]!} \\
                                           & = \frac{\kappa!}{\cnt{x}(\tau'\oplus x)\prod_{x'\in\XX}\left[\cnt{x}(\tau')\right]!}
    ...
    \begin{align*}
          & \EE_\Ss\left[\frac1\kappa E_1(x, x')(\TB)\right]                                                                                                              \\
        = & \frac{1}{z_\Ss}\sum_{\tau\in\tree{1}{\kappa-1}}\one{\tau_o=x'}{\kappa\choose\CNT(\tau)}\psi(\tau)\CF(x, x')\prod_{v\in\NB{\tau}}\CF(\tau_v, \tau_o)           \\
        = & \frac{1}{z_\Ss}\CF(x, x')\sum_{\tau\in\tree{1}{\kappa-1}}\one{\tau_o=x'}{\kappa\choose\CNT(\tau)}\psi(\tau)\CF(x, x')\prod_{v\in\NB{\tau}}\CF(\tau_v, \tau_o) \\
        = & \Es(x, x')                                                                                                                                                    \\
        = & \frac1{z_\Es}\CF(x, x')\CF(x', x).
    \end{align*}
    \begin{align*}
        \frac1{z_\Es}\CF(x', x) & = \frac{1}{z_\Ss}\sum_{\tau\in\tree{1}{\kappa-1}}\one{\tau_o=x'}{\kappa\choose\CNT(\tau)}\psi(\tau)\prod_{v\in\NB{\tau}}\CF(\tau_v, x')     \\
        \CF(x', x)              & = \frac{z_\Es}{z_\Ss}\sum_{\tau\in\tree{1}{\kappa-1}}\one{\tau_o=x'}{\kappa\choose\CNT(\tau)}\psi(\tau)\prod_{v\in\NB{\tau}}\CF(\tau_v, x') \\
    \end{align*}
    Set
    ...
    \begin{equation}
        \CF(x, x') = \frac1{z_\CF}\sum_{\tau\in\tree{1}{\kappa-1}}\one{\tau_o=x}{\kappa\choose\CNT(\tau)}\psi(\tau)\prod_{v\in\NB{\tau}}\CF(\tau_v, x).
    \end{equation}
    ...
    \begin{equation}
        z_\CF = \sum_{x, x'\in\XX}\sum_{\tau\in\tree{1}{\kappa-1}}\one{\tau_o=x}{\kappa\choose\CNT(\tau)}\psi(\tau)\prod_{v\in\NB{\tau}}\CF(\tau_v, x).
    \end{equation}
    Define the functional $\BP: \PP(\XX^2)\mapsto\PP(\XX^2)$ such that
    \begin{equation}
        (\BP\CF)(x, x') = \frac1{z_\CF}\sum_{\tau\in\tree{1}{\kappa-1}}\one{\tau_o=x}{\kappa\choose\CNT(\tau)}\psi(\tau)\prod_{v\in\NB{\tau}}\CF(\tau_v, x),
    \end{equation}
    and $\CF=\BP\CF$.

    To summarize, we found that the stationary points of $\LM(\Ss, \Es, \alpha, \beta, \gamma)$ to be
    \begin{align*}
        \Ss(\tau)  & = \frac{1}{z_\Ss}{\kappa\choose\CNT(\tau)}\psi(\tau)\prod_{v\in\NB{\tau}}\CF(\tau_v, \tau_o) \\
        \Es(x, x') & = \frac1{z_\Es}\CF(x, x')\CF(x', x),
    \end{align*}
    where $\CF=\BP\CF$. These should also be the stationary points of $\VAL$ on the specified domain

    The function value at these points can also be calculated:
    \begin{align*}
        H(\Ss\|\SUs) & = \sum_{\tau\in\tree{1}{\kappa}}\Ss(\tau)\log\frac{\Ss(\tau)}{\SUs(\tau)}                                                                                                                           \\
                     & = \sum_{\tau\in\tree{1}{\kappa}}\Ss(\tau)\log\frac{\frac{1}{z_\Ss}{\kappa\choose\CNT(\tau)}\psi(\tau)\prod_{v\in\NB{\tau}}\CF(\tau_v, \tau_o)}{\frac{1}{|\XX|^{\kappa+1}}{\kappa\choose\CNT(\tau)}} \\
                     & = -\log z_\Ss + (\kappa+1)\log|\XX| + \EE_\Ss\log\psi(\TB) + \EE_\Ss\left[\sum_{v\in\NB{\tau}}\log\CF(\TB_v, \TB_o)\right]
        \intertext{Apply (\ref{Eq.NNNI-EV-SS-ES}) with $g(x, x')=\log h(x, x')$,}
                     & = -\log z_\Ss + (\kappa+1)\log|\XX| + \EE_\Ss\log\psi(\TB) + \kappa\EE_\Es\log\CF(\X, \X').
    \end{align*}
    On the other hand,
    \begin{align*}
        H(\Es\|\EUs) & = \sum_{x, x'\in\XX}\Es(\tau)\log\frac{\Es(\tau)}{\EUs(\tau)}                       \\
                     & = \sum_{x, x'\in\XX}\Es(\tau)\log\frac{\frac1{z_\Es}\CF(x, x')\CF(x',x)}{1/|\XX|^2} \\
                     & = -\log z_\Es + 2\log |\XX| + \EE_\Es\log\CF(\X, \X') + \EE_\Es\log\CF(\X', \X)     \\
        \intertext{By $\Es$ symmetric,}
                     & = -\log z_\Es + 2\log |\XX| + 2\EE_\Es[\log\CF(\X, \X')].
    \end{align*}
    Plugging into $\VAL(\Ss, \Es)$,
    \allowdisplaybreaks{
        \begin{align*}
              & \VAL(\Ss, \Es)                                                                                              \\
            = & \EE_\Ss\log\psi(\TB) - \left[H(\Ss\|\SUs) - \frac{\kappa}{2} H(\Es\|\EUs)\right]                            \\
            = & \EE_\Ss\log\psi(\TB) +\log z_\Ss - (\kappa+1)\log|\XX| - \EE_\Ss\log\psi(\TB) - \kappa\EE_\Es\log\CF(x, x') \\
              & - \frac{\kappa}{2} \log z_\Es + \kappa\log |\XX| + \kappa\EE_\Es\log\CF(x, x')
            \intertext{everything cancels, and\footnote{Notice that we used another expected value property above. No proof was offered.}}
            = & \log z_\Ss - \frac{\kappa}{2} \log z_\Es -\log |\XX|
        \end{align*}}
    This is the value of $\VAL$ for each stationary point. Since for positive configuration $\psi$, the maxima cannot be on a boundary (where some values of the pmf is zero\footnote{I know this is not the case, but I don't really know how to prove this}), we can state the annealed pressure as the following\footnote{I don't really know how to state this. Should I use the KKT conditions?}:
    \begin{equation}
        \AP(\PU) = \sup_{\substack{h\in\PP(\XX^2)\\ \CF=\BP\CF}}\left\{\log\left[\sum_{\tau\in\tree{1}{\kappa}}\psi(\tau)\prod_{v\in\NB{\tau}}h(\tau_v, \tau_o)\right]
        -\frac\kappa2\log\left[\sum_{x, x'\in\XX}h(x, x')h(x', x)\right]\right\}
    \end{equation}
\end{proof}
