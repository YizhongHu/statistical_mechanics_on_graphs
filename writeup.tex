...
\begin{proposition}
    For a continuous function $F: \mathcal{X}^{\kappa+1}\mapsto\mathbb{R}$, $\langle F, \eta \rangle = 0$ for all measures $\eta$ on $\mathcal{X}^{\kappa+1}$ such that
    \begin{enumerate}
        \item $\eta$ is symmetric: for all $x_0, x_1, \cdots, x_\kappa\in\mathcal{X}$ and $\sigma\in S_\kappa$
              \begin{equation*}
                  \eta(x_0, (x_1, \cdots, x_\kappa)) = \eta(x_0, \sigma(x_1, \cdots, x_\kappa)),
              \end{equation*}
        \item and $\pi_\eta=0$: for all $x, y\in \mathcal{X}$
              \begin{equation*}
                  \pi_\eta(x, y) := \frac1\kappa\sum_{v=1}^\kappa\int_{\mathcal{X}^{\kappa-1}}\eta(x, x_1, \cdots, x_{v-1}, y, x_{v+1}, \cdots, x_\kappa)dx_1\cdots dx_{v-1}dx_{v+1}\cdots dx_\kappa = 0
              \end{equation*}
    \end{enumerate}
    if and only if
    \begin{equation*}
    ...
    \end{equation*}
    For some $\gamma: \mathcal{X}^2\mapsto \mathbb{R}$ and $\beta$ such that for all $x_0, x_1, \cdots, x_v\in\mathcal{X}$, $\sigma\in S_\kappa$
    \begin{equation*}
        \sum_{\sigma\in S_\kappa}\beta(x_0, \sigma(x_1, \cdots, x_\kappa)) = 0
    \end{equation*}
\end{proposition}

\begin{proof}
    (Sketch, will use delta functions). For simplicity, we will write $\mathbf{x}:=(x_1, \cdots, x_\kappa)$, and
    ...
    $d^{\kappa-1}_v\mathbf{x}:= dx_1\cdots dx_{v-1}dx_{v+1}\cdots dx_\kappa$.
    ...
    Fix $x_0, x_1, \cdots, x_\kappa\in\mathcal{X}$. Consider
    \begin{equation*}
        \eta_{x_0, \mathbf{x}}(x_0', \mathbf{x}') := \delta(x_0' - x_0)\prod_{v=1}^\kappa\delta(x_v' - x_v)
        - \frac1\kappa\sum_{u=1}^\kappa\delta(x_0' - x_0)\prod_{v=1}^\kappa\delta(x_v' - x_u),
    \end{equation*}
    and
    \begin{equation*}
        \eta(x_0', \mathbf{x}') := \sum_{\sigma\in S_\kappa}\eta_{x_0, \sigma(\mathbf{x})}(x_0', \mathbf{x}').
    \end{equation*}
    Check that $\eta$ is symmetric. On the other hand, to prove that $\pi_\eta=0$,
    \begin{align*}
          & \pi_{\eta_{x_0, \mathbf{x}}} (x, y)                                                                                                                                                      \\
        = & \frac1\kappa\sum_{w=1}^\kappa\int_{\mathcal{X}^{\kappa-1}}\delta(x_0' - x)\delta(x_w' - y)\prod_{v\neq w}\delta(x_v' - x_v)d^{\kappa-1}_w\mathbf{x}'                                     \\
          & - \frac1\kappa\sum_{w=1}^\kappa\frac1\kappa\sum_{u=1}^\kappa\int_{\mathcal{X}^{\kappa-1}}\delta(x_0' - x)\delta(x_w'-y)\prod_{v\neq w}^\kappa\delta(x_v' - x_u)d^{\kappa-1}_w\mathbf{x}' \\
        = & \frac1\kappa\sum_{w=1}^\kappa\delta(x_0' - x)\delta(x_w' - y)
        - \frac1\kappa\sum_{w=1}^\kappa\frac1\kappa\sum_{u=1}^\kappa\delta(x_0' - x)\delta(x_w' - y)                                                                                                 \\
        = & \frac1\kappa\sum_{w=1}^\kappa\delta(x_0' - x)\delta(x_w' - y) -\frac1\kappa\sum_{w=1}^\kappa\delta(x_0' - x)\delta(x_w' - y)=0.
    \end{align*}
    Hence, $\pi_\eta(x, y) = \sum_{\sigma\in S_\kappa}\pi_{\eta_{x_0, \sigma(\mathbf{x})}}(x, y) = 0$.
    ...
    First notice that we can simplify $\eta$:
    \begin{align*}
        \eta(x_0', \mathbf{x}') & = \sum_{\sigma\in S_\kappa}\delta(x_0' - x_0)\prod_{v=1}^\kappa\delta(x_v' - \sigma(\mathbf{x})_v)
        - \sum_{\sigma\in S_\kappa}\frac1\kappa\sum_{u=1}^\kappa\delta(x_0' - x_0)\prod_{v=1}^\kappa\delta(x_v' - \sigma(\mathbf{x})_u)
        \intertext{Take $u' = \sigma^{-1}(u)$, }
                                & = \sum_{\sigma\in S_\kappa}\delta(x_0' - x_0)\prod_{v=1}^\kappa\delta(x_v' - \sigma(\mathbf{x})_v)
        - \sum_{\sigma\in S_\kappa}\frac1\kappa\sum_{u'=1}^\kappa\delta(x_0' - x_0)\prod_{v=1}^\kappa\delta(x_v' - x_{u'})           \\
                                & = \sum_{\sigma\in S_\kappa}\delta(x_0' - x_0)\prod_{v=1}^\kappa\delta(x_v' - \sigma(\mathbf{x})_v)
        - \kappa!\frac1\kappa\sum_{u=1}^\kappa\delta(x_0' - x_0)\prod_{v=1}^\kappa\delta(x_v' - x_{u}).
    \end{align*}
    Further, see that
    \begin{align*}
        \sum_{\sigma\in S_\kappa}\int_{\mathcal{X}^{\kappa+1}}\beta(x_0', \mathbf{x}')\delta(x_0' - x_0)\prod_{v=1}^\kappa\delta(x_v' - \sigma(\mathbf{x})_v)dx_0d^\kappa\mathbf{x}'
         & = \sum_{\sigma\in S_\kappa}\beta(x_0, \sigma(\mathbf{x})) \neq 0 \\
        \int_{\mathcal{X}^{\kappa+1}}\beta(x_0', \mathbf{x}')\delta(x_0' - x_0)\prod_{v=1}^\kappa\delta(x_v' - x_{u})dx_0d^\kappa\mathbf{x}'
         & = \beta(x_0, x_u, \cdots, x_u) = 0
    \end{align*}
    which means that $\langle\beta, \eta\rangle \neq 0$, and since $\langle\frac1\kappa\sum \gamma(\cdot), \eta \rangle = 0$, by linearity,
