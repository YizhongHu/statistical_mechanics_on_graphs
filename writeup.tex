...
\begin{lemma}
    \begin{enumerate}
        The optimizers of the Hardcore Ising optimization problem
        \begin{equation*}
            \psi(\beta, \kappa) = \sup_{\mu\in M} \left\{\mathbb{E}_\mu\left[\beta X_0\right] - \left[H(\mu\|\eta) - \frac\kappa2 H(\pi_\mu\| \pi_\eta)\right]\right\}
        \end{equation*}
        are in the form
        ...
        \begin{equation}
            ...
            \label{Eq.HAIS-h}
        \end{equation}
        and $Z = (1+e^h)^\kappa + e^{\beta + \kappa h}$ the normalizing constant
    \end{enumerate}
\end{lemma}

\begin{proof}
    Leveraging the Lemma 7.1, we optimize for $\tilde{V}$ for $\widehat{\pi}\in \Delta$. We parametrize with (\ref{Eq.HAIS-pi-param}),
    and the gradient is
    \begin{equation}
...
\begin{theorem}
    ...
    \begin{equation*}
        \mu(\mathbf{x}, \tau) = \frac1Z \exp\left[\beta x_0 + h(\beta, \kappa) \sum_{v\in\mathcal{N}_o(\tau)}(1-x_v)\right],
    \end{equation*}
    with $h(\beta, \kappa)$ being the unique solution to
    \begin{equation*}
        h = (\kappa-1)\log(1+e^{-h}) - \beta,
    \end{equation*}
    and $Z = (1+e^h)^\kappa + e^{\beta + \kappa h}$.
...
\begin{proof}
    From Lemma 7.2, we have shown stationary points exists in the form described above.
    In Lemma 7.3, we have shown that for given $\beta$ and $\kappa$, the stationary point exists and is unique.
    It remains to show that the stationary point is a maximum.

    In Lemma 7.1, we have shown that for each edge distribution $\widehat{\pi}$, $\widehat{\mu}$ is the unique maximizer for $V$.
    It is therefore sufficient to show that $\tilde{V}(\beta, \kappa, \widehat{\pi})$ is concave in $\widehat{\pi}$.
    ...
    \end{equation*}
    Therefore, the above stationary point is indeed is a unique maximizer of the objective.
