...
\newcommand{\mMs}{\PP_u(\TT_{*, 1}^{\kappa-1}[\X], \X)}
...
\begin{proof}[Proof of (\ref{Prp.NNNI-CF})]
    ...
    \allowdisplaybreaks{
        \begin{align*}
            ...
            = & \sum_{\tau\in\tree{1}{\kappa}}\frac1\kappa E_1(x, x')(\tau)\frac{1}{z_\Ss}{\kappa\choose\CNT(\tau)}\prod_{\{v, u\}\in\NB{\tau}}\PU(\tau_v, \tau_u)\prod_{v\in\NB{\tau}}\CF(\tau_v, \tau_o)                              \\
            = & \frac{1}{z_\Ss}\sum_{\tau\in\tree{1}{\kappa}}\frac1\kappa \sum_{w\in\NB{\tau}}\one{\tau_w=x, \tau_o=x'}{\kappa\choose\CNT(\tau)}\prod_{\{v, u\}\in\NB{\tau}}\PU(\tau_v, \tau_u)\prod_{v\in\NB{\tau}}\CF(\tau_v, \tau_o)
            \intertext{Introduce $\tau'=\tau(v\BS o)_1$}
            = & \frac{1}{z_\Ss}\sum_{\tau\in\tree{1}{\kappa}}\frac1\kappa \sum_{w\in\NB{\tau}}\sum_{\tau'\in\tree{1}{\kappa-1}}\one{\tau(w\BS o)_1=\tau'}\one{\tau_w=x, \tau_o=x'}                                                      \\
              & {\kappa\choose\CNT(\tau)}\prod_{\{v, u\}\in\NB{\tau}}\PU(\tau_v, \tau_u)\prod_{v\in\NB{\tau}}\CF(\tau_v, \tau_o)
            \intertext{Rewrite everything in terms of $\tau'$}
            = & \frac{1}{z_\Ss}\sum_{\tau\in\tree{1}{\kappa}}\frac1\kappa \sum_{w\in\NB{\tau}}\sum_{\tau'\in\tree{1}{\kappa-1}}\one{\tau=\tau'\oplus x}\one{\tau_w=x, \tau'_o=x'}                                                       \\
            ...
            = & \frac{1}{z_\Ss}\sum_{\tau'\in\tree{1}{\kappa-1}}\sum_{\tau\in\tree{1}{\kappa}}\one{\tau=\tau'\oplus x, \tau'_o=x'}\frac1\kappa \sum_{w\in\NB{\tau}}\one{\tau_w=x}                                                       \\
              & {\kappa\choose\CNT(\tau'\oplus x)}\prod_{v\in\NB{\tau'}}\PU(x, \tau'_v)\prod_{\{v, u\}\in\NB{\tau'}}\PU(\tau'_v, \tau'_u)\ \CF(x, x')\prod_{v\in\NB{\tau'}}\CF(\tau'_v, x')
    ...
    \begin{align*}
        ...
        \Es(x, x') & = \frac1{z_\Es}\frac{\CF(x, x')\CF(x', x)}{\PU(x, x')},
    \end{align*}
    where $\CF=\BP\CF$. These should also be the stationary points of $V_{\PO, \PU}$ on the specified domain

    The function value at these points can also be calculated:
    \begin{align*}
        H(\Ss\|\SUs) & = \sum_{\tau\in\tree{1}{\kappa}}\Ss(\tau)\log\frac{\Ss(\tau)}{\SUs(\tau)}                                                                                                                                                               \\
                     & = \sum_{\tau\in\tree{1}{\kappa}}\Ss(\tau)\log\frac{\frac{1}{z_\Ss}{\kappa\choose\CNT(\tau)}\prod_{\{v, u\}\in\NB{\tau}}\PU(\tau_v, \tau_u)\prod_{v\in\NB{\tau}}\CF(\tau_v, \tau_o)}{\frac{1}{|\X|^{\kappa+1}}{\kappa\choose\CNT(\tau)}} \\
                     & = -\log z_\Ss + (\kappa+1)\log|\X| + \EE_\Ss\left[\sum_{\{v, u\}\in\NB{\tau}}\log\PU(\tau_v, \tau_u)\right] + \EE_\Ss\left[\sum_{v\in\NB{\tau}}\log\CF(\tau_v, \tau_o)\right]                                                           \\
    \end{align*}
    Note that $\EE_\Ss\left[\sum_v \log\CF(\tau_v, \tau_o)\right] = \kappa\EE_\Es\log\CF(x, x')$\footnote{Probably should have proven this ahead 
    of time with all of the other expected value proofs. This is what I mean by the expected value statements can be and should be more general}:
    \allowdisplaybreaks{
        \begin{align*}
              & \kappa\EE_\Es\log\CF(x, x')                                                                                                   \\
            = & \kappa\sum_{x, x'\in\X} \Es(x, x')\log\CF(x, x')                                                                              \\
            = & \kappa\sum_{x, x'\in\X} \sum_{\tau\in\tree{1}{\kappa}}\Ss(\tau)\frac1\kappa E_1(x, x')(\tau)\log\CF(x, x')                    \\
            = & \sum_{x, x'\in\X} \sum_{\tau\in\tree{1}{\kappa}}\Ss(\tau) E_1(x, x')(\tau)\log\CF(x, x')                                      \\
            = & \sum_{x, x'\in\X}\sum_{\tau\in\tree{1}{\kappa}}\Ss(\tau) \sum_{v\in\NB{\tau}}\one{\tau_v=x, \tau_o=x'}\log\CF(x, x')          \\
            = & \sum_{\tau\in\tree{1}{\kappa}}\Ss(\tau) \sum_{v\in\NB{\tau}}\sum_{x, x'\in\X}\one{\tau_v=x, \tau_o=x'}\log\CF(\tau_v, \tau_o) \\
            = & \sum_{\tau\in\tree{1}{\kappa}}\Ss(\tau) \sum_{v\in\NB{\tau}}\log\CF(\tau_v, \tau_o)                                           \\
            = & \EE_\Ss\left[\sum_{v\in\NB{\tau}}\log\CF(\tau_v, \tau_o)\right],
        \end{align*}}
    so,
    \begin{equation*}
        H(\Ss\|\SUs) = -\log z_\Ss + (\kappa+1)\log|\X| + \EE_\Ss\left[\sum_{\{v, u\}\in\NB{\tau}}\log\PU(\tau_v, \tau_u)\right] + \kappa\EE_\Es\log\CF(x, x')
    \end{equation*}
    On the other hand,
    \begin{align*}
        H(\Es\|\EUs) & = \sum_{x, x'\in\X}\Es(\tau)\log\frac{\Es(\tau)}{\EUs(\tau)}                                         \\
                     & = \sum_{x, x'\in\X}\Es(\tau)\log\frac{\frac1{z_\Es}\CF(x, x')\CF(x',x)/\PO(x, x')}{1/|\X|^2}         \\
                     & = -\log z_\Es + 2\log |\X| + \EE_\Es[\log\CF(x, x')] + \EE_\Es\log\CF(x', x) - \EE_\Es\log\PO(x, x') \\
        \intertext{By $\Es$ symmetric,}
                     & = -\log z_\Es + 2\log |\X| + 2\EE_\Es[\log\CF(x, x')] - \EE_\Es\log\PO(x, x').
    \end{align*}
    Plugging into $V_{\PO,\PU}(\Ss, \Es)$,
    \allowdisplaybreaks{
        \begin{align*}
              & V_{\PO,\PU}(\Ss, \Es)                                                                                                                             \\
            = & \EE_\Ss\left[\frac12\sum_{v\in\NB{\tau}}\log\PO(\tau_o, \tau(v\BS o)_0)+\sum_{\{v, u\}\in\NB{\tau}}\log\PU(\tau(v\BS o)_0, \tau(u\BS o)_0)\right] \\
