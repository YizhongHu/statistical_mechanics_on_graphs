...
\begin{proof}
    ...
    To see that the stationary point is indeed a maximum, notice that in (\ref{Eq.HAIS-V-2}), $H(\mu)$ is concave in probability measure $\mu$ and $\sum_{x\in\mathcal{X}}\sum_{k=0}^\kappa \mu(x, k)\log{\kappa\choose k}$ is linear in $\mu$. This gives that $V$ is concave in $\mu$. Thus, by continuity of $V$, all stationary points are local maxima, and the maximum lies on the interior of $M$.

    This means that the above expression, or $\widehat{\mu}$ maximizes the objective function given edge probability measure $\pi_\mu$,
    and is thus the form of an optimizer.

    Hence, given $\widehat{\mu}$ parametrized by $\widehat{\pi}$ we can rewrite the terms
    \begin{align*}
...
\begin{lemma}
    \begin{enumerate}
        An optimizer of the Hardcore Ising optimization problem
        \begin{equation*}
            \psi(\beta, \kappa) = \sup_{\mu\in M} \left\{\mathbb{E}_\mu\left[\beta X_0\right] - \left[H(\mu\|\eta) - \frac\kappa2 H(\pi_\mu\| \pi_\eta)\right]\right\}
        \end{equation*}
        is
...
\begin{proof}
    Leveraging the previous lemma, we optimize for $\tilde{V}$ for $\widehat{\pi}\in \Delta$. We parametrize with (\ref{Eq.HAIS-pi-param}),
    and applying Lagrange multiplier, we have
    \begin{equation}
        \partial \pi^{00} = -\frac\beta2-\frac\kappa2 \left[\log\pi^{00} - \log \frac12(1 - \pi^{00})\right] - \frac{\kappa-1}{2}\log\frac{1-\pi^{00}}{1+\pi^{00}} = 0.
    \end{equation}
...
We can define, based on the previous proposition, we define
\begin{align*}
...
\subsection{Hardcore Ising Annealed Pressure}

The annealed pressure is
\begin{equation*}
    \psi(\beta, \kappa) = \sup_{\mu} \left[\mathbb{E}_\mu(\beta X_0) - H(\mu\|\eta) + \frac\kappa2H(\pi_\mu\|\pi_\eta)\right]
\end{equation*}
which we have shown to be equivalent to
\begin{equation*}
    \psi(\beta, \kappa) = \sup_{\pi_\mu} \left[\mathbb{E}_{\mu_0}(\beta X_0) - (\kappa-1)H(\mu_0) + \frac\kappa2H(\pi_\mu) - \log|\mathcal{X}|\right]
\end{equation*}
Substitute (\ref{Eq.HAIS-pimu-h}), we have
\begin{align*}
    \mathbb{E}_{\mu_0}(\beta X_0) & = \beta \frac{1}{2+e^h}                        \\
    H(\mu_0)                      & = \log(2+e^h) - \frac{1+e^h}{2+e^h}\log(1+e^h) \\
    H(\pi_\mu)                    & = \log(2+e^h) - \frac{he^h}{2+e^h}
\end{align*}
Hence
\begin{equation*}
...
\end{equation*}
Recall (\ref{Eq.HAIS-q-1}). Substitute $q^-=e^h$ and we have
\begin{equation}
