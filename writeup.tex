...
\begin{proposition}
    ...
    \begin{equation*}
        \sup_{\pi^0_*\in\mathcal{P}(\mathcal{X})}\left\{\mathbb{E}_{\pi^0_*}\left[B\cos x_0\right] + (\kappa-1)H(\pi^0_* \| \nu) +
        \frac\kappa2\sup_{\substack{\pi\in\mathcal{P}_s(\mathcal{X}^2)\\ \pi^0=\pi^0_*}}\mathbb{E}_{\pi}[\beta\cos(x_0-x_1)] - H(\pi\|\pi_\eta)\right\}
    \end{equation*}
\end{proposition}
...
\begin{proposition}
    ...
    \begin{equation*}
        \sup_{\substack{\pi\in\mathcal{P}_s(\mathcal{X}^2)\\ \pi^0=\pi^0_*}}\mathbb{E}_{\pi}[\beta\cos(x_0-x_1)] - H(\pi\|\pi_\eta)
    \end{equation*}
    is in the form
    \begin{equation*}
        \frac{d\pi^*}{d\eta}(x, y) = \frac{1}{Z^\pi(h)}\exp[\beta \cos(x - y)]\frac{dh}{d\nu}(x)\frac{dh}{d\nu}(y),
    \end{equation*}
    with some\footnote{Still need to prove that such an $h$ exists. I think we have uniqueness given $\pi^0_*$} $h\in\mathcal{P}(\mathcal{X})$ absolutely continuous with respect to $\nu$ that satisfies, for all $A$ measurable
    \begin{equation}\label{Eq.XY-h-pi0}
        \pi^0_*(A) = \frac{1}{Z^\pi(h)}\int_A\int_\mathcal{X}\exp[\beta \cos(x - y)]h(dx)h(dy),
    \end{equation}
    and
    \begin{equation*}
    ...
    \begin{equation*}
        \log Z^\pi(h) - 2\mathbb{E}_{\pi^0_*}\left[\log\frac{dh}{d\nu}(x)\right].
    \end{equation*}
\end{proposition}

\begin{proof}
    Since $H(\pi\|\pi_\eta)$ is strictly convex and $\mathbb{E}_{\pi}[\beta\cos(x_0-x_1)]$ is linear, the target function is linear.
    Since the set $\{\pi\in\mathcal{P}_s(\mathcal{X}^2)\ |\ \pi^0=\pi^0_*\}$ is compact and simply connected, we know that an optimizer exist and is
    unique.

    With the same strategy, for some $\pi\in\mathcal{P}_s(\mathcal{X}^2)$ such that $\pi^0=\pi^0_*$ we consider the cross entropy
    \begin{align*}
        ...
        \intertext{By $\pi$ symmetric, we know that $\pi^0=\pi^1=\pi^0_*$}
        = & -\log Z^\pi(h) + \mathbb{E}_\pi\left\{\exp[\beta \cos(x - y)]\right\} + 2\mathbb{E}_{\pi^0_*}\left[\log\frac{dh}{d\nu}(x)\right]
    \end{align*}
    Thus,
    \begin{equation*}
        \mathbb{E}_{\pi}[\beta\cos(x_0-x_1)] - \int_{\mathcal{X}^2}\pi(dxdy) \log \frac{d\pi^*}{d\eta}(x, y) = \log Z^\pi(h) - 2\mathbb{E}_{\pi^0_*}\left[\log\frac{dh}{d\nu}(x)\right]
    \end{equation*}
    When $\pi=\pi^*$, we have
    \begin{equation*}
        \mathbb{E}_{\pi^*}[\beta\cos(x_0-x_1)] - H(\pi^*\|\pi_\eta) = \log Z^\pi(h) - 2\mathbb{E}_{\pi^0_*}\left[\log\frac{dh}{d\nu}(x)\right]
    \end{equation*}
    Similarly, we invoke Lemma \ref{Prp.XY-RE-GE-CE}, and get that for all $\pi\in\mathcal{P}_s(\mathcal{X}^2)$ such that $\pi^0=\pi^0_*$,
    \begin{align*}
...
\begin{proposition}
    ...
    is equivalent to
    \begin{equation*}
        \sup_{h\in\mathcal{P}(\mathcal{X})}\left\{\mathbb{E}_{\pi^0_*}\left[B\cos x_0\right] + (\kappa-1)H(\pi^0_* \| \nu) +
        \frac\kappa2\log Z^\pi(h) - \kappa\mathbb{E}_{\pi^0_*}\left[\log\frac{dh}{d\nu}(x)\right]\right\}
    \end{equation*}
\end{proposition}
